...
@card(['header' => ''])
    <form role="form" action="{{ route('projects.update', $projects->id_project) }}" method="POST" class="form-material">
        ...
        <div class="form-group">
            <input name="name" value="{{ $projects->name }}" type="text" class="form-control {{ $errors->has('name') ? 'is-invalid':'' }}" placeholder="Project Name">
        </div>
        <div class="form-group">
            <textarea name="description" id="description" cols="5" rows="5" class="form-control {{ $errors->has('description') ? 'is-invalid':'' }}" placeholder="Description">{{ $projects->description }}</textarea>
        </div>
